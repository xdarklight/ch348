...
static void ch348_update_msr(struct usb_serial_port *port, u8 msr)
{
	...
	dev_err(&port->dev, "got MSR = 0x%02x\n", msr); // HACK
...
static void ch348_process_status_urb(struct usb_serial *serial, struct urb *urb)
{
	...
	for (i = 0; i < urb->actual_length;) {
		...
		if (status_entry->reg_iir == R_INIT) {
			...
			dev_err(&port->dev, "VEN_R status with control = 0x%02x\n", status_entry->data.ven_r_msr.control);
...
static int ch348_set_modem_control(struct usb_serial_port *port, u8 mcr)
{
	...
	dev_err(&port->dev, "DTR = %s, RTS = %s\n", str_on_off(dtr), str_on_off(rts)); // HACK
	WARN_ON(!dtr && !rts); // HACK
	...
	dev_err(&port->dev, "done setting MCR\n"); // HACK
			      ...
			      const struct ktermios *termios_old)
{
	...
	dev_err(&port->dev, "set_termios baudrate = %u\n", baudrate); // HACK
...
static int ch348_tiocmget(struct tty_struct *tty)
{
	...
	result |= mcr & UART_MCR_RTS ? TIOCM_RTS : 0;
	result |= msr & UART_MSR_CTS ? TIOCM_CTS : 0;
	result |= msr & UART_MSR_DCD ? TIOCM_CAR : 0;
	result |= msr & UART_MSR_RI ? TIOCM_RI : 0;
	result |= msr & UART_MSR_DSR ? TIOCM_DSR : 0;
			  ...
			  unsigned int clear)
{
	...
	dev_err(&port->dev, "tiocmset: mcr_set = 0x%02x, mcr_clear = 0x%02x\n", mcr_set, mcr_clear); // HACK
