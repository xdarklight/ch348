...
#define CH348_NUM_GPIO_SMALL_PACKAGE		12
...
static void ch348_process_status_urb(struct usb_serial *serial, struct urb *urb)
{
	...
	for (i = 0; i < urb->actual_length;) {
		...
		if (!status_entry->reg_iir) {
			...
			status_len += sizeof(status_entry->gpio_in_mask);

			gpio_in_val = le64_to_cpu(status_entry->gpio_in_mask);
			bitmap_from_arr64(ch348->gpio_in_mask, &gpio_in_val,
					  ch348->gc.ngpio);
			...
			status_len += sizeof(status_entry->lsr_signal);
				    ...
				    unsigned long *bits)
{
	...
	scoped_guard(mutex, &ch348->gpio_value_lock) {
		for_each_set_bit(i, mask, gc->ngpio) {
			if (test_bit(i, bits) &&
			    !test_bit(i, ch348_gpio_input_only_mask))
				set_bit(i, ch348->gpio_out_mask);
			else
				clear_bit(i, ch348->gpio_out_mask);
		}
...
static int ch348_attach(struct usb_serial *serial)
{
	...
	ch348->gc.label = "ch348";
	ch348->gc.base = -1;
	ch348->gc.can_sleep = true;
	ch348->gc.names = ch348_gpio_names;

	if (ch348->small_package)
		ch348->gc.ngpio = CH348_NUM_GPIO_SMALL_PACKAGE;
	else
